...
class MyModelLoss(LossInterface):
    ...
    def get_loss_G(self, run_dict, valid=False):
        ...
        if "W_ADV" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_ADV"]:
            L_adv = (-run_dict["g_pred_fake"]).mean()
            L_G += self.CONFIG["LOSS"]["W_ADV"] * L_adv
            self.loss_dict["L_g_adv"] = round(L_adv.item(), 4)

        if "W_ID" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_ID"]:
        ...
        if "W_VGG" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_VGG"]:
        ...
        if "W_LPIPS" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_LPIPS"]:
        ...
        if "W_L1" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_L1"]:
        ...
        if "W_RECON" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_RECON"]:
        ...
        if "W_CYCLE" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_CYCLE"]:
        ...
        if "W_FEAT" in self.CONFIG["LOSS"] and self.CONFIG["LOSS"]["W_FEAT"]:
